...
let playerName = "";
let oldId = "PersonalAccount"

function UpdateAccount(divId, bal)
{
    $("#" + divId + "_Balance").html("$" + bal.toLocaleString() + ".00");
}
...
function AddAccount(account_name, account_type, bal, ply_Name, ply_Citizenid, atm, prefix)
{
    ...
    var accData = {
        ...
        bal : bal,
        ply_Name : ply_Name,
        ply_Citizenid : ply_Citizenid,
        prefix : prefix,
        temp_Name : temp_Name
    }

    $("#Player_Accounts .hula").append(`<div class='bgdark2 ${transferAcc == `personal` ? `selected` : ``}' id="${temp_Name}">
        <div class='account'>
            ...
            <div class='d-flex justify-content-between' style='position: relative;bottom: 5vh'>
                ${atm ? `` : `<button type='button' class='text-muted deposit' id='DepositModal' onClick='ResetModals("${transferAcc}")'>DEPOSIT</button>`}
                <button style="${atm ? `margin-left: -1vh;` : ``}" type='button' class='text-muted withdraw' id='WithdrawModal' onClick='ResetModals("${transferAcc}")'>WITHDRAW</button>
                <button type='button' class='text-muted transfer' id='TransferModal' onClick='ResetModals("${transferAcc}")'>TRANSFER</button>
            </div>
        </div>
    </div>`);
    $("#"+temp_Name).data('AccountData', accData)

    ValidAccounts[temp_Name] = transferAcc;
}

$(document).on('click', '.bgdark2', function(e){
    var Data = $(this).data('AccountData');
    var PressedAcc = Data.temp_Name

    if (PressedAcc != oldId) {
        $("#Transaction_Row").find(".kula").fadeOut(10).css({'display':'none'})
        setTimeout( () => {
            $("#Transaction_Row .kula").html("");
            $("#Transaction_Row").find(".roda").css({'display':'flex'})
            $("#Transaction_Row .roda").find(".lds-spinner").css({'display':'inline-block'})
        },11)
        setTimeout( () => {
            $("#Transaction_Row .roda").find(".lds-spinner").css({'display':'none'})
            $("#Transaction_Row").find(".roda").css({'display':'none'})
        }, 300)

        $("#Transaction_Row").find(".kula").fadeIn(10).css({'display':'block'})

        if (Data.prefix != undefined && Data.prefix != null) {
            CurrentTransactions = [];
            $.post("https://" + folder_name + "/ChangeAcc", JSON.stringify({
                    account: Data.prefix,
            }));
        } else {
            CurrentTransactions = [];
            $.post("https://" + folder_name + "/ChangeAcc", JSON.stringify({
                    account: Data.ply_Citizenid,
            }));
        }
        $("#"+oldId).removeClass('selected')
        $("#"+PressedAcc).addClass('selected')

        oldId = PressedAcc
    }
});

// ${house.locked == 1 ? "fa-lock" : "fa-lock-open"}
...
function OpenATM(data, transactions, name, citizenid, cash, atm)
{
    if (data && data !== null)
    {
        let tbl = JSON.parse(data);
        $("#Player_Accounts .hula").html("");
        for (var i = 0; i < tbl.length; i++)
        {
            let tTbl = tbl[i];
            AddAccount((tTbl.type === "business" && tTbl.name || tTbl.type === "organization" && tTbl.name || "Personal Account"), (lang[tTbl.type] && lang[tTbl.type] || tTbl.type), tTbl.amount, name, citizenid, atm, tTbl.prefix);
        }
    }
    ...
    $("#bankui").css({'height':'200', 'margin-top':'calc((1080px - 200px)/2)'});


    var calc = (1080 - 810)/2
    $("#bankui").css({'display':'flex'});
    $('#bankui').find('.lds-spinner').css({'display':'inline-block'});
    setTimeout( () => {
        $('#bankui').find('.lds-spinner').css({'display':'none'});
        $("#bankui").css({'display':'block'}).animate({
            height: "810px",
            marginTop: calc + "px",
        }, 500);

        $("#bankui").css({'display':'block'});
        $(".show-content").fadeTo(10, 1.0)
    },2000)
}
...
$(document).on('click', '#DepositModal', function(e){
    e.preventDefault();
    var Data = $(this).parent().parent().parent().data('AccountData');
    $('.deposit-container').css({'display':'block'});
    $('.deposit-account').text(Data.account_name+" / "+Data.ply_Citizenid);
...
$(document).on('click', '.deposit-submit', function(e){
    ...
    $('.items-view').css({'display':'none'});
    $('.deposit-container').find('.lds-spinner').css({'display':'inline-block'});
    ...
    setTimeout( () => {
        ...
        setTimeout( () => {
            $('.deposit-container').find('.lds-spinner').css({'display':'none'});
...
$(document).on('click', '#WithdrawModal', function(e){
    ...
    $('.withdraw-account').text(Data.account_name+" / "+Data.ply_Citizenid);
    $('.items-view').css({'display':'block'});
    $('.full-white').css({'display':'block'});
});
...
$(document).on('click', '.withdraw-submit', function(e){
    ...
    $('.items-view').css({'display':'none'});
    $('.withdraw-container').find('.lds-spinner').css({'display':'inline-block'});
    ...
    setTimeout( () => {
        ...
        setTimeout( () => {
            $('.withdraw-container').find('.lds-spinner').css({'display':'none'});
...
$(document).on('click', '#TransferModal', function(e){
    ...
    $('.transfer-account').text(Data.account_name+" / "+Data.ply_Citizenid);
    $('.items-view').css({'display':'block'});
    $('.full-white').css({'display':'block'});
});
...
$(document).on('click', '.transfer-submit', function(e){
    ...
    $('.items-view').css({'display':'none'});
    $('.transfer-container').find('.lds-spinner').css({'display':'inline-block'});
    ...
    setTimeout( () => {
        ...
        setTimeout( () => {
            $('.transfer-container').find('.lds-spinner').css({'display':'none'});
...
{
    let name = data.name;
    OpenATM(data.accounts, data.transactions, name, data.citizenid, data.cash, data.atm);
}
    ...
    document.onkeyup = function(data){
        if (data.which == 27){
            if ($(".deposit-container").is(':visible')) {
                ...
                $(".show-content").fadeTo(10, 0.0)
                $("#bankui").css({'display':'none'});
                $.post("https://" + folder_name + "/CloseATM", JSON.stringify({}));
                CurrentTransactions = [];
                oldId = "PersonalAccount"
            }
