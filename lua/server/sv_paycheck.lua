RegisterServerEvent('qb-banking:server:sendPaycheck', function(pAmount, pSource)
    local src = pSource
...
RegisterServerEvent('qb-banking:server:Paycheck:pickup', function()
    local src = source
    if not src then return end
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if data ~= nil then
        ...
        if (paycheck > 0) then
            ...
            AddTransaction(src, "personal", player.PlayerData.citizenid, paycheck, "deposit", "N/A", (note ~= "" and note or ""), uuid)
